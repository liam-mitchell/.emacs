...
(flycheck-def-args-var flycheck-cl-args c/c++-cl
  :package-version '(flycheck . "0.22"))
...
(flycheck-define-checker c/c++-cl
  "Syntax checker definition for Microsoft's cl.exe"
  :command ("cl" (eval flycheck-cl-args) "/Zs" source)
  :error-patterns
...
(puthash 'c/c++-cl "/Zs" flycheck-checker-syntax-args)
...
(defun flycheck-command-p (alist)
  ...
  (string-match-p
   ...
   (file-truename (cdr (assoc 'file alist)))))
...
Matches against FLYCHECK-CHECKER-EXECUTABLE-REGEXPS to determine a match."
  (let ((regexp (gethash checker flycheck-checker-executable-regexps)))
    (if (not (null regexp))
        (string-match-p regexp (file-name-nondirectory command))
      nil)))
...
(defun flycheck-split-and-unquote (command)
  (let ((lst (split-string-and-unquote command))
        (split-command '()))
    (while (not (null lst))
      (let ((elem (pop lst)))
        (if (string-match-p ".*=$" elem)
            (setq split-command (append split-command (cons (concat elem (pop lst)) nil)))
          (setq split-command (append split-command (cons elem nil))))))
    split-command))
...
Finally, gathers error patterns for `automatic' by matching the executable from
 ...
 for the checker we're actually using."
  ...
  (let* ((alist (flycheck-read-command flycheck-automatic-command-file))
         (command (flycheck-split-and-unquote (cdr (assoc 'command alist))))
    ...
    (put 'automatic 'flycheck-predicate (lambda () t))))
